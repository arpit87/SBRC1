...
public class SBChatService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (Platform.getInstance().isLoggingEnabled()) Log.i("LocalService", "Received start id " + startId + ": " + intent);
...
class SBChatBroadcastReceiver extends BroadcastReceiver{
	...
	{
		if (Platform.getInstance().isLoggingEnabled()) Log.i(TAG,"update intent in chat rece ,might broadcast");
		...
		for (NearbyUser n:nearbyUserList)
		{
			...
			if(!fbid.equals(""))
				try {	
					...
					if(chatManager != null)					
					{
						if (Platform.getInstance().isLoggingEnabled()) Log.i(TAG,"broadcasting to fbid:"+fbid);
						chatManager.getChat(fbid).sendMessage(msg);
					}
				} catch (RemoteException e) {
					if (Platform.getInstance().isLoggingEnabled()) Log.i(TAG,"Unable to send broadcast msg");
					e.printStackTrace();
				}
		}
